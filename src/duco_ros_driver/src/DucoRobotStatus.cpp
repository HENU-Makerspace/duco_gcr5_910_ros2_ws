...
void DucoRobotStatus::getRobotStatus()
{
    ...
    if( controller_connected_flag_ )
    {
        for(int axis=0;axis<arm_num;axis++)
        {
            ...
            if (robot_joint_actual_acc[axis].size() < _dof)
            {
            ...
            if (robot_joint_actual_torq[axis].size() < _dof)
            {
                RCLCPP_ERROR(this->get_logger(),"Actual joint torque data size incorrect!");
                return;
            }

            _last_joints_actual_pos[axis].assign(robot_joint_actual_pos[axis].begin(), robot_joint_actual_pos[axis].begin() + _dof);
            _last_joints_actual_vel[axis].assign(robot_joint_actual_vel[axis].begin(), robot_joint_actual_vel[axis].begin() + _dof);
            _last_joints_actual_acc[axis].assign(robot_joint_actual_acc[axis].begin(), robot_joint_actual_acc[axis].begin() + _dof);
            _last_joints_actual_torq[axis].assign(robot_joint_actual_torq[axis].begin(), robot_joint_actual_torq[axis].begin() + _dof);
            ...
            auto has_status_values = [this](const auto & values, const char * name) {
                if (values.size() < _dof)
                {
                    RCLCPP_ERROR(this->get_logger(), "Robot status field '%s' data size incorrect!", name);
                    return false;
                }
                return true;
            };
            if (!has_status_values(status.jointExpectPosition, "jointExpectPosition") ||
                !has_status_values(status.jointExpectVelocity, "jointExpectVelocity") ||
                !has_status_values(status.jointExpectAccelera, "jointExpectAccelera") ||
                !has_status_values(status.jointActualPosition, "jointActualPosition") ||
                !has_status_values(status.jointActualVelocity, "jointActualVelocity") ||
                !has_status_values(status.jointActualAccelera, "jointActualAccelera") ||
                !has_status_values(status.jointActualCurrent, "jointActualCurrent") ||
                !has_status_values(status.cartExpectPosition, "cartExpectPosition") ||
                !has_status_values(status.cartExpectVelocity, "cartExpectVelocity") ||
                !has_status_values(status.cartExpectAccelera, "cartExpectAccelera") ||
                !has_status_values(status.cartActualPosition, "cartActualPosition") ||
                !has_status_values(status.cartActualVelocity, "cartActualVelocity") ||
                !has_status_values(status.cartActualAccelera, "cartActualAccelera") ||
                !has_status_values(status.jointTemperature, "jointTemperature"))
            {
                return;
            }
            ...
            for( uint i=0;i<_dof;++i)
            {
                ...
                robot_state_.slave_ready[i] = i < status.slaveReady.size() ? status.slaveReady[i] : false;

                robot_state_.joint_temperature[i] = status.jointTemperature[i];
                robot_state_.driver_temperature[i] = i < status.driverTemperature.size() ? status.driverTemperature[i] : 0.0;
            }
